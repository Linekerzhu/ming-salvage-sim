...
class WebGame:
    ...
    def _absorb_eunuch_lore_from_text(self, minister_name: str, text: str, *, source_role: str = "") -> Dict[str, Any]:
        ...
        if semantic_targets is not None:
            ...
            if not self._dialogue_lore_regex_fallback_enabled():
                return {}
    ...
    def _fallback_pending_directive(
        ...
        if not self._dialogue_directive_regex_fallback_enabled():
            return None
    ...
    def _castration_consent_note(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        for row in self.db.list_minister_stances(turn=self.state.turn, minister_name=name, limit=12):
            ...
            if action_kind != "castration":
                continue
            if not bool(psychological.get("explicit_consent")):
                continue
            latest_relevant = row
    ...
    def _dialogue_route_context(self, minister_name: str, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "semantic_route_enabled": self._dialogue_route_llm_audit_available(),
    ...
    def _dialogue_regex_summon_response(self, minister_name: str, text: str) -> Optional[Dict[str, Any]]:
        if not self._dialogue_regex_summons_enabled():
            return None
        deterministic_summon = self._attendant_summon_target(minister_name, text)
        if not deterministic_summon:
            return None
    ...
    def _dialogue_pending_action_from_route(
        self,
        minister_name: str,
        text: str,
        decision: SemanticDecision,
    ...
    def _dialogue_initial_semantic_response(
        self,
        minister_name: str,
        text: str,
        *,
        ...
        decision = self._dialogue_user_semantic_decision(minister_name, text)
        response = self._dialogue_response_from_user_semantic_decision(
            minister_name,
            text,
            decision,
            chat_turn_id=chat_turn_id,
    ...
    def _dialogue_user_semantic_decision(self, minister_name: str, text: str) -> SemanticDecision:
        try:
            character = self.session._character(minister_name)
            pending = self._load_pending_dialogue_action(minister_name)
            route_context = self._dialogue_route_context(minister_name, text)
            recent_answers = self._recent_minister_answer_texts(minister_name, limit=4)
            return self._dialogue_semantic_engine().evaluate_user_message(
                character,
                text,
                pending_action=pending if isinstance(pending, dict) and pending else None,
                route_context=route_context,
    ...
    def _dialogue_response_from_user_semantic_decision(
        self,
        minister_name: str,
        text: str,
        decision: SemanticDecision,
        *,
        ...
        if decision.decision_type == "action":
            return self._dialogue_semantic_action_response_from_decision(
                minister_name,
                text,
                decision,
                chat_turn_id=chat_turn_id,
        ...
        *,
        ...
        if phase == "confirm":
            ...
            return self._execute_semantic_dialogue_action(
                ...
                review=review,
                chat_turn_id=chat_turn_id,
                decision_type=decision.decision_type or "action",
            )
        if phase != "propose":
            return None
        action = self._action_from_semantic_decision(minister_name, text, decision)
    ...
    def _dialogue_semantic_action_response(
        self,
        minister_name: str,
        text: str,
        *,
    ...
    def _dialogue_semantic_recovery_response(
        self,
        minister_name: str,
        text: str,
        *,
    ...
    def _dialogue_pending_action_response_from_decision(
        self,
        minister_name: str,
        text: str,
        decision: SemanticDecision,
        *,
        ...
        if action_type == "recruitment":
            ...
            return self._execute_semantic_dialogue_action(
                ...
                review=review,
                chat_turn_id=chat_turn_id,
                decision_type=decision.decision_type or "pending",
            )
    ...
    def _dialogue_semantic_pending_action_response(
        self,
        minister_name: str,
        text: str,
        *,
        ...
        Pending actions are already structured.  This path only asks the
        targeted audit whether the latest player utterance confirms that pending
        action; it replaces the old "准/照办" keyword confirmation path.
        """
    ...
    def _semantic_pending_recovery_action(self, minister_name: str, text: str) -> Dict[str, Any]:
        if not self._dialogue_action_llm_audit_available():
            return {}
        if self._load_pending_dialogue_action(minister_name):
            return {}
    ...
    def _recover_pending_dialogue_action_from_recent_answer(self, minister_name: str, text: str) -> Dict[str, Any]:
        ...
        semantic_action = self._semantic_pending_recovery_action(minister_name, text)
        if semantic_action:
            return semantic_action
        if not self._dialogue_pending_regex_recovery_enabled():
            return {}
        if self._dialogue_action_llm_audit_available():
            return {}
    ...
    def _record_unknown_dialogue_mentions(self, minister_name: str, answer: str) -> None:
        if (
            not self._dialogue_mention_llm_audit_available()
            and not self._dialogue_mention_regex_fallback_enabled()
        ):
            return
    ...
    def _execute_dialogue_consequence_action(
        self,
        minister_name: str,
        action: Dict[str, Any],
        *,
    ...
    def _execute_dialogue_action(
        ...
        *,
        ...
        if action.get("type") == "dialogue_consequence":
            return self._execute_dialogue_consequence_action(
                minister_name,
                action,
                chat_turn_id=chat_turn_id,
            )
    ...
    def _dialogue_action_response(
        ...
        *,
        ...
        pending = self._load_pending_dialogue_action(minister_name)
        if pending:
            return self._dialogue_semantic_pending_action_response(
                minister_name,
                text,
                chat_turn_id=chat_turn_id,
            )
    ...
    def _directive_chat_effect(
        ...
        try:
            ...
            if semantic_review is None and not self._dialogue_directive_regex_fallback_enabled():
                return {}
    ...
    def chat(self, minister_name: str, message: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        dialogue_response = self._dialogue_initial_semantic_response(
            minister_name,
            text,
            chat_turn_id=chat_turn_id,
        )
    ...
    def _local_quick_suggestion_fallback_enabled(self) -> bool:
        return os.environ.get("MING_SIM_ENABLE_LOCAL_QUICK_SUGGESTIONS", "").strip().lower() in ("1", "true", "yes")
    ...
    def suggestions_for(self, character: Character) -> List[Dict[str, Any]]:
        ...
        if "check_treasury" in skill_ids:
            seed_suggestions.insert(min(1, len(seed_suggestions)), {"label": "问钱粮", "text": "太仓和内库实数如何？本月哪些钱最急？"})
        if "check_military" in skill_ids or "front_line_plan" in skill_ids or "strategic_review" in skill_ids:
            seed_suggestions.insert(min(1, len(seed_suggestions)), {"label": "问军情", "text": "关宁军、京营和陕西边军的士气、欠饷与补给，眼下哪处最危险？"})
        if "secret_investigation" in skill_ids:
            seed_suggestions.insert(min(1, len(seed_suggestions)), {"label": "要线索", "text": "若要暗查，哪些账册、人物和证据最该先摸清？"})
