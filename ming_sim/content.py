...
from ming_sim.constants import BUILDING_CATEGORIES, BUILDING_OUTPUT_METRICS, CONTENT_DIR
...
def _content_asset_signature() -> Tuple[Tuple[str, int, int], ...]:
    ...
    for root, dirs, files in os.walk(CONTENT_DIR):
        ...
        for filename in files:
            ...
            try:
                ...
                continue
            records.append((os.path.relpath(path, CONTENT_DIR), int(stat.st_mtime_ns), int(stat.st_size)))
...
class GameContent:
    ...
    def load(cls) -> "GameContent":
        if cls is not GameContent:
            return cls._load_uncached()
        return _cached_game_content(_content_asset_signature()).runtime_copy()

    @classmethod
    def clear_load_cache(cls) -> None:
        _cached_game_content.cache_clear()

    def runtime_copy(self) -> "GameContent":
        """返回供单局运行使用的副本：会变的盘面对象独立复制，大型只读知识层共享。"""
        events = copy.deepcopy(self.events)
        seed_events = copy.deepcopy(self.seed_events)
        return GameContent(
            factions=copy.deepcopy(self.factions),
            characters=copy.deepcopy(self.characters),
            npc_network=self.npc_network,
            npc_tiangang=self.npc_tiangang,
            events=events,
            seed_events=seed_events,
            opening_legacies=copy.deepcopy(self.opening_legacies),
            event_by_id={ev.id: ev for ev in (*events, *seed_events)},
            regions=copy.deepcopy(self.regions),
            armies=copy.deepcopy(self.armies),
            buildings=copy.deepcopy(self.buildings),
            faction_metrics=tuple(self.faction_metrics),
            powers=copy.deepcopy(self.powers),
            classes=copy.deepcopy(self.classes),
            office_skills=copy.deepcopy(self.office_skills),
            skill_catalog=self.skill_catalog,
            office_default_skills=copy.deepcopy(self.office_default_skills),
            personal_skill_ids=copy.deepcopy(self.personal_skill_ids),
            common_skills=list(self.common_skills),
            skill_descriptions=self.skill_descriptions,
            grant_keywords=self.grant_keywords,
            directive_keywords=self.directive_keywords,
            directive_skill_ids=set(self.directive_skill_ids),
            office_definitions=self.office_definitions,
            skill_tool_templates=self.skill_tool_templates,
            game_world_prompt=self.game_world_prompt,
            minister_agent_prompt=self.minister_agent_prompt,
            consort_agent_prompt=self.consort_agent_prompt,
            decree_writer_prompt=self.decree_writer_prompt,
            season_simulator_prompt=self.season_simulator_prompt,
            score_extractor_shared_prompt=self.score_extractor_shared_prompt,
            score_extractor_module_prompts=self.score_extractor_module_prompts,
            chapter_memory_prompt=self.chapter_memory_prompt,
            ending_summary_prompt=self.ending_summary_prompt,
            fiscal_items=copy.deepcopy(self.fiscal_items),
            items=copy.deepcopy(self.items),
            adventures=copy.deepcopy(self.adventures),
        )

    @classmethod
    def _load_uncached(cls) -> "GameContent":
...
@lru_cache(maxsize=4)
def _cached_game_content(_signature: Tuple[Tuple[str, int, int], ...]) -> GameContent:
    return GameContent._load_uncached()
