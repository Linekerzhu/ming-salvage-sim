...
核心原则：
- 这是语义判定，不按“净身、宫刑、净身房、入内廷”等词机械触发。
- allow=true 只用于旨意本身明确命令对 target_name 执行净身、宫刑、腐刑、去势、押赴净身房、没入内廷为奴等身份处置。
- 普通调查、制度改革、查净身房弊端、询问旧例、传闻、假设、比较方案、禁止执行、暂缓、不许惊动净身房，必须 allow=false。
- 目标人物只能从 candidate_names 中选择；不要把承办人 assignee 当成目标，除非旨意明确命令处置承办人本人。
- 如果一句话只是“查某人净身旧例/查净身房/不要把某人净身/若某人净身会怎样”，allow=false。
