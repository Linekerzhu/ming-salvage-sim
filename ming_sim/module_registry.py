...
_SPECS: Tuple[ModuleSpec, ...] = (
    ...
    _module(
        "mechanic.statecraft_center",
        kind="mechanic",
        owner="ming_sim.statecraft_center",
        entrypoint="statecraft_center_payload",
        hooks=(_hook("statecraft.read", timing="observe", contract="fiscal + bureaucracy + buildings -> state-machine panel"),),
        depends_on=("mechanic.bureaucracy", "mechanic.fixed_period_flows"),
        reads=("FiscalCenter payload", "organization diagnostics", "buildings", "GameState"),
        writes=("statecraft panel payload",),
        hot_swap="runtime_safe",
        risk="medium",
        notes="HOI-style explanation layer; must not mutate economy or offices.",
    ),
    _module(
