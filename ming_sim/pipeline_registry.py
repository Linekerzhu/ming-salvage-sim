...
_SPECS: Tuple[PipelineSpec, ...] = (
    ...
    _spec(
        ...
        kind="llm",
        owner="ming_sim.rule_audit",
        entrypoint="directive_castration_execution_audit",
        reads=("completed directive text", "assignee", "candidate characters"),
        writes=("court-immediate castration execution gate",),
        llm_role="dialogue_audit",
        default_max_tokens=800,
        failure_policy="fail_closed",
        cache_policy="none",
        risk="high",
        notes="旨意办结后的强制净身/宫刑执行必须经语义审计；失败不转换身份、不写净身旧档。",
    ),
    _spec(
        "llm.piaoni-writer",
        kind="llm",
    ...
    _spec(
        ...
        kind="llm",
        owner="ming_sim.dialogue_audit",
        entrypoint="dialogue_eunuch_lore_intake_audit",
        reads=("player/NPC text", "NPC behavior context", "mentioned character candidates", "pending dialogue action"),
        writes=("eunuch lore intake gate",),
        llm_role="dialogue_audit",
        default_max_tokens=900,
        failure_policy="fail_closed",
        cache_policy="none",
    ...
    _spec(
        ...
        kind="llm",
        owner="ming_sim.dialogue_audit",
        entrypoint="dialogue_unknown_mention_intake_audit",
        reads=("NPC answer text", "candidate extracted names", "NPC behavior context"),
        writes=("unknown dialogue mention gate",),
        llm_role="dialogue_audit",
        default_max_tokens=900,
        failure_policy="fail_closed",
        cache_policy="none",
        risk="high",
        notes="NPC 回答中的新人物候选/暗示召见必须经语义审计；失败不入候选池、不自动召见。",
    ),
    _spec(
        "llm.decree_writer",
        kind="llm",
