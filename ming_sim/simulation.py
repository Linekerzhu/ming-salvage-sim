...
from ming_sim.bureaucracy import (
    compact_bureaucracy_brief,
    directive_execution_assessments,
...
TOP_LEVEL_ALIASES = {
    ...
    "人物身体变化": "condition_changes",
    "身疾刑伤": "condition_changes",
    "人物刑罚": "punishment_changes",
    "刑罚执行": "punishment_changes",
...
ITEM_FIELD_ALIASES = {
    ...
    "force_castration": "force_castration", "强制净身": "force_castration", "强旨净身": "force_castration",
    "force_emancipation": "force_emancipation", "强制脱籍": "force_emancipation", "强旨脱籍": "force_emancipation",
    "conversion_kind": "conversion_kind", "身份转换": "conversion_kind",
    "condition_key": "condition_key", "状况编号": "condition_key",
    "label": "label", "病名": "label", "伤名": "label", "状况": "label",
    "system": "system", "系统": "system",
    "severity": "severity", "严重度": "severity",
    "stage": "stage", "病程": "stage", "伤势": "stage",
    "effects": "effects", "身体影响": "effects", "奏对影响": "effects",
    "hidden": "hidden", "隐藏": "hidden",
    "chronic": "chronic", "慢性": "chronic",
    "duration_days": "duration_days", "持续天数": "duration_days",
    "fatal": "fatal", "致死": "fatal", "死亡": "fatal",
    "taxonomy": "taxonomy", "体系": "taxonomy", "刑罚体系": "taxonomy",
    "punishment_key": "punishment_key", "刑罚键": "punishment_key",
    "punishment": "punishment", "刑名": "label", "刑罚": "label",
    "executor": "executor", "执行机关": "executor",
    "agency": "agency", "机关": "agency", "司法机关": "agency",
    "facility": "facility", "狱名": "facility", "prison": "facility",
    "pressure": "pressure", "威逼": "pressure", "拷掠强度": "pressure",
    "coercion_goal": "coercion_goal", "逼令": "coercion_goal", "强令": "coercion_goal", "供状目标": "coercion_goal",
...
ITEM_FIELD_LABELS = {
    ...
    "force_castration": "强制净身",
    "force_emancipation": "强制脱籍",
    "conversion_kind": "身份转换",
    "faction": "派系",
    "status": "状态",
    "office": "位号",
    "office_type": "官署类别",
...
def _directive_context(
    state: GameState,
    db: GameDB,
    directives: Optional[List[sqlite3.Row]],
) -> List[Dict[str, object]]:
    """每道旨意的承办画像，供月末推演判执行风险。"""
    ...
    for index, row in enumerate(directives or [], 1):
        ...
        item: Dict[str, object] = {
            ...
            "event_title": str(_row_value(row, "event_title", "") or ""),
            "actor": actor,
            "text": str(_row_value(row, "text", "") or "")[:260],
        }
        if not actor:
            item["execution_note"] = "未指定拟旨/承办大臣；按诏书正文、职掌与名册推定承办人。"
            rows.append(item)
            continue
...
def build_simulator_payload(
    ...
    return {
        ...
        "bureaucracy_brief": compact_bureaucracy_brief(bureaucracy),
        "execution_assessments": execution_assessments,
        "court_bureaucracy_audit": {
            "方案比选": [
