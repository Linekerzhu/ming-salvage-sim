...
- 例外：secret_order 包含一次性密令建档、对既有密令的明确催办/加急/即核、玩家明确准许承办宦官按净身旧患调整差遣策略，以及玩家明确要求/准许承办人回报并落档本月进展或提交待核议；office_change 是一次性任免动作。只有玩家本轮明确“下密令/密旨/命某人暗查某事”、明确要求某条密令“催办/加急/本月即核”、明确准许某条密令采用副手换班/避触发/先调养/强派等旧患差遣策略、明确要求“说进展/入档/提交核议”，或明确任命/授官/补某人某缺时，才可在没有 pending_action 时返回 phase=confirm 并允许即时落库。
...
- secret_order：下达新密令时，只有玩家明确下达密令/密旨，且能从玩家原话读出承办人或承办对象、暗查/取证/盯梢等任务目标，才可 allow=true 且 phase=confirm。催办既有密令时，tool_action.kind/mode/action 为 rush 且含 order_id，必须玩家原话明确要求该密令“催办/加急/限期/本月即核”，才可 allow=true，kind="rush"，phase=confirm。净身旧患差遣策略时，tool_action.kind/action 为 dispatch_strategy 且含 order_id/strategy，必须玩家原话明确准许“副手换班/分班轮值/避开刑房净房/先调养再派/照旧强派”等具体策略，才可 allow=true，kind="dispatch_strategy"，mode=工具策略。查办进展时，tool_action.kind=progress 且含 order_id/progress，必须玩家原话明确要求承办人“回奏/说说/呈上/入档本月进展”，才可 allow=true，kind="progress"；提交核议时，tool_action.kind=submit_review 且含 order_id/claim，必须玩家原话明确同意“办到位就提交核议/送核/候月末核议”，才可 allow=true，kind="submit_review"。只是问“要不要暗查”“此事能否密办”“何时能办完”、NPC 自行建议密查/催办/差遣策略/提交核议，必须 false。
...
- “这条密令查到哪了？” + rush_secret_order 工具 => allow=false。
- “把 #3 这条密令本月即核。” + rush_secret_order 工具 => allow=true, phase=confirm, action_type=secret_order, kind=rush。
- “你看王承恩旧患该怎么办？” + set_eunuch_dispatch_strategy 工具 => allow=false。
- “准，#3 用副手轮值，别硬撑坏事。” + set_eunuch_dispatch_strategy 工具 => allow=true, phase=confirm, action_type=secret_order, kind=dispatch_strategy, mode=relay。
- “此事何时能办完？” + report_secret_order_progress 工具 => allow=false。
- “说说 #3 本月查到什么，照实入档。” + report_secret_order_progress 工具 => allow=true, phase=confirm, action_type=secret_order, kind=progress。
- “若已办到位，就提交核议。” + submit_secret_order_for_review 工具 => allow=true, phase=confirm, action_type=secret_order, kind=submit_review。
...
任务：阅读 NPC 回答、舞台动作、皇帝原话或工具上下文，判断候选姓名里哪些是真正可写入“未知人物候选池/可召见候选”的具体人物。
...
- purpose=route_summon 时必须同时满足：皇帝本轮原话明确要传/召/带这个名册外具体姓名入殿，且该姓名就在 candidate_names 中；不能因为 route 审计拟召见就自动补档。
