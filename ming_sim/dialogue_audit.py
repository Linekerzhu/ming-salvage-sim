...
- 例外：secret_order 包含一次性密令建档、对既有密令的明确催办/加急/即核、以及玩家明确准许承办宦官按净身旧患调整差遣策略；office_change 是一次性任免动作。只有玩家本轮明确“下密令/密旨/命某人暗查某事”、明确要求某条密令“催办/加急/本月即核”、明确准许某条密令采用副手换班/避触发/先调养/强派等旧患差遣策略，或明确任命/授官/补某人某缺时，才可在没有 pending_action 时返回 phase=confirm 并允许即时落库。
