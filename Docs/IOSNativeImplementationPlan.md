...
关键纠偏：第一版不能在这些基建尚未连成图谱时直接增加游戏运行层。当前数据已经能支撑群星谱展示，但还不能稳定支撑政策结算。P0 必须先把 NPC、官署、官职、地域和风险打通，再进入财政纵切。

## P0 数据基座优先原则

P0 的交付物不是玩法，而是 Foundation Graph：一张能被 Swift 规则系统查询的“人物-官署-岗位-地域-风险-关系”基础图。

当前审计结论：

```text
已有：263 名 NPC、460 条任官记录、1555 条人物关系边。
已有：15 个一级区、203 个府级节点、63 个机构、115 个官职。
已有：群星谱能展示人物卡、官职上下文、天罡、命数、心盘、势网。
不足：NPC 籍贯仍是中文文本，没有桥接到正式 region_id。
不足：官职能连到 office post，但没有形成岗位容量、占员、缺口和管辖图。
不足：地区没有系统性风险初始盘面，无法承载财政、灾荒、治安、边防压力。
不足：人物能力、价值观、派系和关系网还没有映射到治理领域和执行阻力。
不足：省级三司、府州主官与行政区之间尚未形成可查询的管辖网络。
```

因此，后续任何玩法里程碑必须遵守：

```text
没有 Foundation Graph，不做月末结算。
没有地区风险基线，不做财政危机纵切。
没有岗位容量和占员图，不做人事任免。
没有 NPC-治理领域映射，不做承办人评分。
没有关系网政治含义，不把势网用于执行背书。
```

P0 完成后，群星谱应该从“人物展示器”升级为“基座审计台”：玩家和开发者都能看到一个 NPC 为什么可召见、为什么适合某类事务、他属于哪些官署/地域/派系网络，以及这些连接是否已经校验。
...
建议第一批先搭数据基座骨架，而不是先搭玩法骨架：

```text
Models/Foundation/FoundationGraph.swift
Models/Foundation/NPCFoundationNode.swift
Models/Foundation/InstitutionFoundationNode.swift
Models/Foundation/RegionFoundationNode.swift
Models/Foundation/OfficeSlotNode.swift
Models/Foundation/GovernanceDomain.swift
Models/Foundation/SystemicRiskBaseline.swift
Services/FoundationGraphBuilder.swift
Services/FoundationGraphAuditService.swift
Views/Foundation/FoundationAuditView.swift
Views/Foundation/NPCFoundationDetailView.swift
Views/Foundation/RegionFoundationDetailView.swift
```
...
第一批不要拆太碎。P0 只追求把静态种子连接成可查询图谱；等 Foundation Graph 稳定后，再进入 P1-P4 的玩法骨架。

## P0-P4 具体实施顺序

### Step 0：完成 Foundation Graph

最小改动：

```text
读取 NPCDatabase + EnvironmentDatabase
生成 NPCFoundationNode：生平摘要、价值观、能力、心盘、势网、现任/旧任官职
生成 InstitutionFoundationNode：机构、官职、岗位容量、现任占员、缺口
生成 RegionFoundationNode：行政层级、上级、下级、管辖官职、风险基线
生成 CrossLink：NPC ↔ 官职 ↔ 机构 ↔ 地域 ↔ 风险 ↔ 关系网
新增 FoundationGraphAuditService
群星谱增加“基座连接”视角或内部审计视图
```

完成即验收：

- 263 名 NPC 都能生成 `NPCFoundationNode`。
- 15 个一级区和 203 个府级节点都能生成 `RegionFoundationNode`。
- NPC 籍贯至少能桥接到省级 `region_id`，核心 NPC 尽量桥接到府级。
- 所有 active office holder 能查到机构、官职、品秩、岗位容量。
- 六部、内阁、司礼监、厂卫、边镇、地方三司有治理领域标签。
- 每个一级区有财政、灾荒、治安、边防/流民等至少 4 类风险基线。
- 关系网可以按举荐、弹劾、执行、庇护、敌对压力给出政治解释。
- 审计脚本和 App 内审计视图都能指出缺口，而不是静默吞掉。
...
### M0 Foundation Graph 数据基座

目标：把群星谱背后的静态数据整理成可查询、可审计、可结算的基础图谱。

交付物：

- `FoundationGraph`
- `NPCFoundationNode`
- `InstitutionFoundationNode`
- `OfficeSlotNode`
- `RegionFoundationNode`
- `GovernanceDomain`
- `SystemicRiskBaseline`
- `FoundationGraphBuilder`
- `FoundationGraphAuditService`
- `FoundationAuditView`

验收标准：

- NPC 基座覆盖人物生平、价值观、能力、心盘、关系网、任官状态。
- 环境基座覆盖正式行政层级、机构、官职、岗位容量、管辖关系和风险基线。
- NPC 与环境之间至少打通：籍贯、现任/旧任官职、机构、治理领域、派系/关系网影响。
- Foundation Graph 构建失败时给出明确缺口，不允许玩法层继续初始化。
- 群星谱能展示一个人物的“基座连接状态”，例如官署、岗位、地域、关系网和治理领域是否已校验。
...
目标：在 Foundation Graph 通过审计后，把基础图谱接入第一版游戏状态。

交付物：

- `ScenarioSeed`
- `FoundationGraphSnapshot`
- `GameSessionState`
- `GameMonth`
- `RuntimeNPCState`
- `RuntimeRegionState`
...
- 启动新局后可从 Bundle seed 构建 Foundation Graph，再构建运行时状态。
- Foundation Graph 审计不通过时，进入基座审计页，不进入玩法页。
...
目标：完成 NPC 数据基座、环境数据基座和二者之间的 Foundation Graph。P0 没过，不进入玩法开发。

完成标准：

- 263 名 NPC 都有可查询的生平摘要、价值观/天罡、能力、心盘、关系网、任官状态。
- 15 个一级区、203 个府级节点都有正式父子关系、风险基线和管辖入口。
- 机构和官职能形成岗位容量、当前占员、空缺和治理领域。
- NPC 的籍贯、任官、派系、关系网至少能桥接到省级/机构级；核心 NPC 桥接到府级或关键边镇。
- 省级三司、府州主官、六部、内阁、司礼监、厂卫等能被 Foundation Graph 查询。
- `FoundationGraphAuditService` 和脚本审计都能给出缺口报告。
- 群星谱能作为 P0 验收界面，显示人物与官署、地域、风险、关系网的连接状态。
